...
def build_image(profile, pull: bool = True):
    ...
    cmd = [
        ...
        f"{imagename}:{tag}",
        "-f",
        f'dockerfiles/Dockerfile.{profile["name"]}',
        ".",
    ]
    if pull:
        cmd.insert(-1, "--pull")
...
def tag_image(profile, version_tag):
    ...
    subprocess.run(
        ["docker", "tag", f"{imagename}:{default_tag}", f"{imagename}:{new_tag}"],
        check=True,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tag-latest", action="store_true", help="Tag build images with 'latest'"
    )
    ...
    parser.add_argument(
        "--tag-branch", action="store_true", help="Tag images with git branch name"
    )
    parser.add_argument(
        "--no-pull", action="store_true", help="Don't pull base images during build"
    )
